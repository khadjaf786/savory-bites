...
module {
  ...
  type NewActor = {
    ...
    dishes : Map.Map<DishId, Dish>;
    orders : Map.Map<OrderId, Order>;
    state : { var nextOrderId : Nat };
  };

  public func migration(_old : {}) : NewActor {
    let dishes = Map.empty<DishId, Dish>();
    for (dish in seedDishes().values()) {
  ...
  // First-run menu. Inlined here because migration files may not import
  // project modules; this is the single place the menu is seeded.
